...
function getOptionsFromEnv(cwd?: string, config?: Config): AnalyzeOptions {
	...
	const effectiveConfig =
		config ?? (cwd ? loadConfig(cwd) : DEFAULT_TEST_CONFIG);
	return {
		cwd,
		config: effectiveConfig,
		strict: envTruthy("SAFETY_NET_STRICT"),
		paranoidRm:
			envTruthy("SAFETY_NET_PARANOID") || envTruthy("SAFETY_NET_PARANOID_RM"),
		paranoidInterpreters:
			envTruthy("SAFETY_NET_PARANOID") ||
			envTruthy("SAFETY_NET_PARANOID_INTERPRETERS"),
	};
}
